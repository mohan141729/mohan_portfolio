...
const Hero = () => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 w-full h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Mobile Layout */}
        <div className="lg:hidden flex flex-col items-center justify-center h-full py-8 sm:py-12">
          ...
          <div className="flex flex-col justify-center items-center text-center max-w-4xl mx-auto">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold mb-3 sm:mb-4 text-white leading-tight drop-shadow">
              {content.subtitle} <span className="text-blue-400">{content.name}</span>
            </h1>
            
            <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold mb-4 sm:mb-6 text-blue-200">
              {content.title}
            </h2>
            
            <p className="max-w-2xl text-white/90 mb-6 sm:mb-8 text-base sm:text-lg leading-relaxed">
              {content.description}
            </p>
        ...
        <div className="hidden lg:flex items-center justify-between h-full">
          ...
          <div className="flex-1 flex flex-col justify-center items-start text-left max-w-2xl">
            <div className="inline-block bg-white/10 text-base text-blue-300 px-5 py-2 rounded-full mb-6 shadow-lg backdrop-blur">
              {content.welcome_text}
            </div>
            
            <h1 className="text-6xl lg:text-7xl xl:text-8xl font-extrabold mb-4 text-white leading-tight drop-shadow">
              {content.subtitle} <span className="text-blue-400">{content.name}</span>
            </h1>
            
            <h2 className="text-3xl lg:text-4xl xl:text-5xl font-semibold mb-6 text-blue-200">
              {content.title}
            </h2>
            
            <p className="text-xl lg:text-2xl text-white/90 mb-8 leading-relaxed max-w-xl">
              {content.description}
            </p>
            ...
            <div className="flex gap-6 mb-8">
              <a 
                href="#projects" 
                className="px-10 py-4 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl shadow-lg hover:from-blue-600 hover:to-purple-600 text-white font-bold transition-all duration-300 text-lg hover:scale-105"
              >
                View Projects
              </a>
              <button
                onClick={handleDownloadResume}
                disabled={!resume}
                className={`px-10 py-4 rounded-xl font-bold transition-all duration-300 border-0 outline-none focus:ring-2 focus:ring-blue-300 text-lg
                  ${resume
                    ? 'bg-gradient-to-r from-emerald-500 to-blue-500 text-white shadow-lg hover:from-emerald-600 hover:to-blue-600 hover:scale-105'
                    : 'bg-gray-300 text-gray-400 cursor-not-allowed'}
                `}
            ...
            <div className="flex gap-8 text-3xl">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="hover:text-blue-400 transition-all duration-300 flex items-center gap-3 hover:scale-110"
              >
                <i className="fab fa-github"></i> 
          ...
          <div className="flex-1 flex justify-end items-center">
            <div className="relative bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8 flex flex-col items-center border border-white/20 animate-float">
              ...
              <div className="relative mb-6">
                <span className="absolute -inset-8 rounded-full bg-gradient-to-tr from-blue-400 via-purple-400 to-pink-400 blur-lg opacity-60"></span>
                <img
                  src={getImageUrl(content.profile_image)}
                  alt={content.name}
                  className="w-80 h-80 xl:w-96 xl:h-96 object-cover rounded-full border-8 border-white shadow-xl relative z-10"
                />
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-white drop-shadow">{content.name}</div>
                <div className="text-blue-200 text-lg font-medium">Full-Stack Developer</div>
              </div>
            </div>
          </div>
        </div>
